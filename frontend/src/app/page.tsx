  ...
  return (
    <main className="min-h-screen bg-white text-[#222222]">
      ...
      <header className="sticky top-0 z-40 bg-white border-b border-gray-200">
        ...
        <div className="h-[82px] px-6 lg:px-12 flex items-center justify-between">
          ...
          <button
            ...
            className="text-[#FF385C] text-[30px] font-bold tracking-[-1.5px]"
          >
            airbnb
          </button>
          ...
          <nav className="hidden md:flex items-center gap-8 absolute left-1/2 -translate-x-1/2">
            ...
            {categories.map((category) => (
              <button
                ...
                className={`flex flex-col items-center gap-1 px-3 py-2 border-b-2 transition ${
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="hidden md:block text-sm font-semibold hover:bg-gray-100 px-4 py-3 rounded-full"
            >
              Become a host
            </button>
            ...
            <button
              ...
              className="hidden sm:flex w-10 h-10 items-center justify-center rounded-full hover:bg-gray-100 transition"
            ...
            <div className="relative">
              ...
              <button
                ...
                className="flex items-center gap-3 border border-gray-300 rounded-full px-3 py-2 hover:shadow-md transition"
              >
                <Menu size={18} />
              ...
              {menuOpen && (
                <div className="absolute right-0 top-14 bg-white border border-gray-200 rounded-2xl shadow-xl w-56 py-2 overflow-hidden">
      ...
      {languageOpen && (
        <div
          className="fixed inset-0 z-[100] bg-black/40 backdrop-blur-[1px] flex items-center justify-center p-4 sm:p-6"
          onMouseDown={(event) => {
            if (event.target === event.currentTarget) setLanguageOpen(false);
          }}
        >
          <div className="w-full max-w-[1030px] max-h-[88vh] overflow-hidden rounded-[24px] bg-white shadow-2xl">
            <div className="flex items-center justify-between px-6 sm:px-7 pt-6">
              <button onClick={() => setLanguageOpen(false)} className="w-9 h-9 rounded-full flex items-center justify-center hover:bg-gray-100 transition" aria-label="Close">
                <X size={22} />
              </button>
              <div className="w-9" />
            </div>
            ...
            <div className="px-6 sm:px-7">
              <div className="flex items-end gap-8 border-b border-gray-200">
                <button onClick={() => setLanguageTab("language")} className={`pb-4 pt-2 text-sm font-semibold border-b-2 ${languageTab === "language" ? "border-black text-[#222]" : "border-transparent text-gray-500"}`}>Language and region</button>
                <button onClick={() => setLanguageTab("currency")} className={`pb-4 pt-2 text-sm font-semibold border-b-2 ${languageTab === "currency" ? "border-black text-[#222]" : "border-transparent text-gray-500"}`}>Currency</button>
              </div>
            </div>

            <div className="overflow-y-auto max-h-[calc(88vh-115px)] px-6 sm:px-7 pb-10">
              {languageTab === "language" ? (
                <>
                  <div className="mt-8 rounded-2xl bg-[#f7f7f7] p-4 sm:p-5 flex items-center justify-between gap-6">
                    <div>
                      <div className="font-medium">Translation ▧</div>
                      <p className="mt-1 text-sm text-gray-500">Automatically translate descriptions and reviews to English.</p>
                    </div>
                    <button onClick={() => setTranslationEnabled(v => !v)} className={`relative h-8 w-14 shrink-0 rounded-full transition ${translationEnabled ? "bg-[#222]" : "bg-gray-300"}`} aria-label="Toggle translation">
                      <span className={`absolute top-1 h-6 w-6 rounded-full bg-white shadow-sm transition-all ${translationEnabled ? "left-7" : "left-1"}`} />
                    </button>
                  </div>

                  <section className="mt-9">
                    <h2 className="text-[24px] sm:text-[26px] font-semibold">Suggested languages and regions</h2>
                    <div className="mt-6 grid grid-cols-2 sm:grid-cols-5 gap-x-8 gap-y-5">
                      {[
                        ["English", "United States"], ["English", "United Kingdom"], ["हिन्दी", "भारत"], ["ಕನ್ನಡ", "ಭಾರತ"], ["मराठी", "भारत"]
                      ].map(([language, region]) => (
                        <button key={`${language}-${region}`} onClick={() => { setSelectedLanguage(language); setSelectedRegion(region); }} className="text-left rounded-xl p-2 -m-2 hover:bg-gray-50 transition">
                          <div className="text-[15px] font-medium">{language}</div><div className="text-[15px] text-gray-500">{region}</div>
                        </button>
                      ))}
                    </div>
                  </section>
                  ...
                  <section className="mt-10">
                    <h2 className="text-[24px] sm:text-[26px] font-semibold">Choose a language and region</h2>
                    <div className="mt-5 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-x-7 gap-y-1">
                      {[
                        ["English", "India"], ["Azərbaycan dili", "Azərbaycan"], ["Bahasa Indonesia", "Indonesia"], ["Bosanski", "Bosna i Hercegovina"], ["Català", "España"],
                        ["Čeština", "Česká republika"], ["Crnogorski", "Crna Gora"], ["Dansk", "Danmark"], ["Deutsch", "Deutschland"], ["Deutsch", "Österreich"],
                        ["Deutsch", "Schweiz"], ["Deutsch", "Luxemburg"], ["Eesti", "Eesti"], ["English", "Australia"], ["English", "Canada"],
                        ["English", "Guyana"], ["English", "Ireland"], ["English", "New Zealand"], ["English", "Singapore"], ["English", "United Arab Emirates"]
                      ].map(([language, region]) => {
                        const active = language === selectedLanguage && region === selectedRegion;
                        return <button key={`${language}-${region}`} onClick={() => { setSelectedLanguage(language); setSelectedRegion(region); }} className={`text-left rounded-xl px-3 py-3 min-h-[62px] border transition ${active ? "border-black" : "border-transparent hover:bg-gray-50"}`}>
                          <div className="text-[15px] leading-5">{language}</div><div className="text-[14px] text-gray-500 leading-5">{region}</div>
                        </button>;
                      })}
                    </div>
                  </section>
                ...
                <section className="py-9">
                  <h2 className="text-[24px] sm:text-[26px] font-semibold">Choose a currency</h2>
                  <p className="mt-2 text-gray-500 text-sm">Prices will be displayed in your selected currency.</p>
                  <div className="mt-7 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                    {[
                      ["INR", "Indian rupee"], ["USD", "United States dollar"], ["GBP", "British pound"], ["EUR", "Euro"],
                      ["AUD", "Australian dollar"], ["CAD", "Canadian dollar"], ["AED", "United Arab Emirates dirham"], ["SGD", "Singapore dollar"]
                    ].map(([code, name]) => <button key={code} onClick={() => setSelectedCurrency(code)} className={`text-left rounded-xl border p-4 transition ${selectedCurrency === code ? "border-black shadow-sm" : "border-gray-200 hover:border-black"}`}><div className="font-semibold">{code}</div><div className="text-sm text-gray-500 mt-1">{name}</div></button>)}
                  </div>
                </section>
              )}
            </div>
        ...
        <div className="px-6 pb-5">
          ...
          <button
            ...
            className="mx-auto flex items-center w-full max-w-[860px] rounded-full border border-gray-300 shadow-md hover:shadow-lg transition bg-white text-left"
          >
            ...
            <div className="flex-1 px-7 py-3 hidden sm:block">
            ...
            <div className="mr-2 w-12 h-12 rounded-full bg-[#FF385C] flex items-center justify-center text-white shrink-0">
              <Search size={20} />
            </div>
      ...
      {searchOpen && (
        <div className="fixed inset-0 z-[100] bg-black/40 flex items-start justify-center pt-24 px-4">
          ...
          <div className="relative bg-white rounded-[28px] shadow-2xl w-full max-w-[880px] overflow-hidden">
            ...
            <div className="p-7">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div className="border border-gray-300 rounded-2xl p-5 hover:border-black transition">
                ...
                <div className="border border-gray-300 rounded-2xl p-5">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="w-10 h-10 border border-gray-400 rounded-full flex items-center justify-center hover:border-black"
                    >
                      <Minus size={16} />
                    </button>
              ...
              <button
                ...
                className="mt-5 w-full bg-[#FF385C] text-white py-4 rounded-xl font-semibold flex items-center justify-center gap-2 hover:bg-[#E31C5F] transition"
              >
                <Search size={20} />
